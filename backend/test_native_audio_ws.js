...
async function testWS() {
  ...
  console.log(`Connecting to ${url} for ${modelName}...`);
  ...
  socket.addEventListener('message', async (event) => {
    ...
    console.log('Received message size:', payloadText.length);
    try {
      ...
      if (msg.error) {
        ...
        socket.send(JSON.stringify({ realtimeInput: { text: 'Hello, respond with exactly "NATIVE AUDIO WS OK"' } }));
        ...
        for (const part of parts) {
          ...
          if (part.inlineData) {
            console.log(`Received Audio MIME: ${part.inlineData.mimeType}, size: ${part.inlineData.data?.length}`);
          }
        }
